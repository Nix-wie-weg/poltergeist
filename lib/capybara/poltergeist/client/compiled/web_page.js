...
Poltergeist.WebPage = (function() {
  ...
  function WebPage(_native) {
    ...
    this._temp_headers = {};
    this._temp_not_for_redirect_headers = {};
    this.frames = [];
  ...
  WebPage.prototype.onInitializedNative = function() {
    ...
    this.removeTempNotForRedirectHeaders();
  ...
  WebPage.prototype.addTempNotForRedirectHeader = function(header) {
    var name, value, _results;
    _results = [];
    for (name in header) {
      value = header[name];
      _results.push(this._temp_not_for_redirect_headers[name] = value);
    }
    return _results;
  };
  ...
  WebPage.prototype.removeTempNotForRedirectHeaders = function() {
    var allHeaders, name, value, _ref2;
    allHeaders = this.getCustomHeaders();
    _ref2 = this._temp_not_for_redirect_headers;
    for (name in _ref2) {
      value = _ref2[name];
      delete allHeaders[name];
    }
